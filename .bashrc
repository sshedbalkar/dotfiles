...
basic_prompt() {
  ...
  if [ "$color_prompt" = yes ]; then
      prompt="${debian_chroot:+($debian_chroot)}\033[01;32m$USER@$(hostname)\033[00m:\033[01;34m$curdir\033[00m"
  else
      prompt="${debian_chroot:+($debian_chroot)}$USER@$(hostname):$curdir"
  fi

  # If this is an xterm set the title to user@host:dir
  case "$TERM" in
  xterm*|rxvt*)
      prompt="\e]0;${debian_chroot:+($debian_chroot)}$USER@$(hostname): $curdir\a$prompt"
      ;;
  *)
      ;;
  esac
